...
export async function GET(request: NextRequest) {
  try {
    ...
    const ALLOWED_STATUSES = ['PENDING', 'IN_REPAIR', 'QUOTE', 'REPAIRED', 'NOT_REPAIRED', 'WRITTEN_OFF', 'RETURNED_TO_CLIENT'] as const
    ...
    if (!status || status === 'ACTIVE') {
      jobs = await prisma.$queryRaw<any[]>`
        ...
        WHERE j.status IN ('PENDING', 'IN_REPAIR', 'QUOTE')
        ORDER BY
          CASE j.status WHEN 'IN_REPAIR' THEN 0 WHEN 'QUOTE' THEN 1 WHEN 'PENDING' THEN 2 ELSE 3 END,
          j."sentAt" ASC
      `
    } else if (status === 'ALL') {
      jobs = await prisma.$queryRaw<any[]>`
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { type, itemId, clientId, problem, serialNumberId } = await request.json()
    ...
    if (type === 'STOCK') {
      ...
      if (item.tracksSerialNumbers && serialNumberId) {
        ...
        await prisma.$executeRaw`
          ...
          VALUES (${repairJobId}, ${repairRef}, 'STOCK', ${itemId}, ${serialNumberId}, 1, 'PENDING', ${problem.trim()}, ${payload.userId}, ${now}::timestamptz, ${now}::timestamptz, ${now}::timestamptz)
        `
      } else {
        await prisma.$executeRaw`
          INSERT INTO "PartRepairJob" (id, reference, type, "itemId", quantity, status, problem, "sentById", "sentAt", "createdAt", "updatedAt")
          VALUES (${repairJobId}, ${repairRef}, 'STOCK', ${itemId}, 1, 'PENDING', ${problem.trim()}, ${payload.userId}, ${now}::timestamptz, ${now}::timestamptz, ${now}::timestamptz)
        `
      }
      ...
      await prisma.$executeRaw`
        INSERT INTO "PartRepairJob" (id, reference, type, "itemId", quantity, status, problem, "deliveredToClientId", "sentById", "sentAt", "createdAt", "updatedAt")
        VALUES (${repairJobId}, ${repairRef}, 'CLIENT', ${itemId}, 1, 'PENDING', ${problem.trim()}, ${resolvedClientId}, ${payload.userId}, ${now}::timestamptz, ${now}::timestamptz, ${now}::timestamptz)
      `
