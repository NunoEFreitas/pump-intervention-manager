...
export async function GET(
  ...
  try {
    ...
    const photos = await prisma.$queryRaw<{ id: string; filename: string; mimeType: string; createdAt: Date }[]>`
      SELECT id, filename, "mimeType", "createdAt" FROM "RepairJobPhoto"
      WHERE "jobId" = ${id}
      ORDER BY "createdAt" ASC
    `
    return NextResponse.json(photos)
  } catch (error) {
    console.error('Error fetching repair photos:', error)
...
export async function POST(
  ...
  try {
    ...
    await prisma.$executeRaw`
      INSERT INTO "RepairJobPhoto" (id, "jobId", filename, "mimeType", data, "createdAt", "createdById")
      VALUES (${photoId}, ${jobId}, ${filename}, ${detectedMime}, ${safeData}, ${now}::timestamptz, ${payload.userId})
    `
