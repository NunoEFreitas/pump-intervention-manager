...
export default function AdminEquipmentBrandsPage() {
  ...
  const [items, setItems] = useState<EquipmentBrand[]>([])
  const [newName, setNewName] = useState('')
  const [editing, setEditing] = useState<EquipmentBrand | null>(null)
  const [saving, setSaving] = useState(false)

  useEffect(() => {
    const userStr = localStorage.getItem('user')
    if (userStr && JSON.parse(userStr).role !== 'ADMIN') { router.push(`/${locale}/dashboard`); return }
    fetch('/api/admin/equipment-brands', { headers: { Authorization: `Bearer ${localStorage.getItem('token')}` } })
  ...
  const save = async () => {
    ...
    try {
      ...
      const url = editing ? `/api/admin/equipment-brands/${editing.id}` : '/api/admin/equipment-brands'
  ...
  const del = async (id: string) => {
    ...
    const token = localStorage.getItem('token')
    const res = await fetch(`/api/admin/equipment-brands/${id}`, { method: 'DELETE', headers: { Authorization: `Bearer ${token}` } })
  ...
  return (
    <div>
      <div className="flex items-center gap-3 mb-6">
        ...
        <h1 className="text-2xl font-bold text-gray-900">{t('equipmentBrands')}</h1>
      </div>

      <div className="card">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-lg font-semibold text-gray-900">{t('equipmentBrands')}</h2>
          <button onClick={() => setNewName(' ')} className="btn btn-primary text-sm">{t('addBrand')}</button>
        </div>

        {newName !== '' && !editing && (
          <div className="flex gap-2 mb-4">
            <input type="text" className="input text-gray-800 flex-1" placeholder={t('brandName')} value={newName.trim() === '' ? '' : newName} autoFocus
        ...
        {items.length === 0 ? <p className="text-gray-500 text-sm py-2">{t('noBrands')}</p> : (
