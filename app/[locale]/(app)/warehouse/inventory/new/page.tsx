...
export default function NewInventoryPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push(`/${locale}/warehouse/inventory/${data.id}`)
    } catch {
  ...
  return (
    <div className="max-w-lg mx-auto">
      <button onClick={() => router.push(`/${locale}/warehouse`)} className="text-blue-600 hover:text-blue-800 mb-4 text-sm">
        ← Voltar
      </button>
      ...
      <form onSubmit={handleSubmit} className="card space-y-5">
        ...
        <div className="flex gap-3">
          ...
          <button type="button" onClick={() => router.push(`/${locale}/warehouse`)} className="btn btn-secondary">
            Cancelar
          </button>
        </div>
      </form>
